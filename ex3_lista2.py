...
class regionGrow():
    ...
    def ApplyRegionGrow(self):
        ...
        for x0 in range (self.h):
            for y0 in range (self.w):
                ...
                (int(self.im[x0,y0,0])*
                int(self.im[x0,y0,1])*int(self.im[x0,y0,2]) > 0) :  
    ...
    def BFS(self, x0,y0):
        ...
        elems=[]
        elems.append((int(self.im[x0,y0,0])+
        int(self.im[x0,y0,1])+
        int(self.im[x0,y0,2]))/3)
        var=self.thresh
        neighbours=self.getNeighbour(x0,y0)
        
        for x,y in neighbours:
            if self.passedBy[x,y] 
            == 0 and self.distance(x,y,x0,y0)<var:
                if(self.PassedAll()):
                    break;
                self.passedBy[x,y] = regionNum
                self.stack.push((x,y))
                elems.append((int(self.im[x,y,0])+
                int(self.im[x,y,1])+int(self.im[x,y,2]))/3)
                var=np.var(elems)
    ...
    def PassedAll(self):
        ...
        return self.iterations>200000 or 
        np.count_nonzero(self.passedBy > 0)
        == self.w*self.h
    ...
    def distance(self,x,y,x0,y0):
        return ((int(self.im[x,y,0])
        -int(self.im[x0,y0,0]))**2+(int(self.im[x,y,1])-
        int(self.im[x0,y0,1]))**2+(int(self.im[x,y,2])
        -int(self.im[x0,y0,2]))**2)**0.5

exemple = regionGrow("apple.jpeg",12)
